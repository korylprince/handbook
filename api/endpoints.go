...
//missingListHandler will return LDAPUsers who have not signed if the sessionID is valid
//or an HTTP 401 Error if not.
func missingListHandler(c *Context, w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	if sess != nil && sess.User != nil && sess.User.Admin {
		...
		ldapList, err := c.LDAPDB.List()
		if err != nil {
			handleError(w, http.StatusInternalServerError, fmt.Errorf("Error getting list from LDAP: %v", err))
			return
		}

		//get list of signed usernames
		signed := make(map[string]struct{})
		for _, u := range list {
			signed[u.Username] = struct{}{}
		}

		var missing []*LDAPUser
		for _, u := range ldapList {
			if _, ok := signed[u.Username]; !ok {
				missing = append(missing, u)
			}
		}

		e := json.NewEncoder(w)
		err = e.Encode(MissingListResponse{List: missing})
		if err != nil {
			handleError(w, http.StatusInternalServerError, fmt.Errorf("Error encoding json: %v", err))
		}
		return
